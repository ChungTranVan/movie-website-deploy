    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent>
        ...
        <Box component="form" sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          <TextField label="Số tập" name="episode_number" type="number" value={form.episode_number} onChange={handleChange} sx={{ width: 100 }} />
          <TextField label="Tiêu đề" name="title" value={form.title} onChange={handleChange} sx={{ flex: 1, minWidth: 200 }} />
          <TextField label="Video URL" name="video_url" value={form.video_url} onChange={handleChange} sx={{ flex: 1, minWidth: 200 }} />
          <TextField label="Subtitle URL" name="subtitle_url" value={form.subtitle_url} onChange={handleChange} sx={{ flex: 1, minWidth: 200 }} />
          <Button variant="contained" onClick={handleSubmit}>{editId ? 'Cập nhật' : 'Thêm mới'}</Button>
